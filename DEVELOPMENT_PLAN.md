...
- [ ] Remaining §14 client attachment (run half): final-feedback → `updateSubmission`/`markCorrect` ordering (§14.3, contract pinned in the compat appendix), autosaver (`saveFile` debounce), event logging on user actions, mark-submitted/share/force-load quick-menu wiring.
