...
- [x] Minified editor variant (§8.4) — `chrome/MinifiedEditor.tsx`: text-only by default (blocks via `mode` prop), short height, inline Run/Stop/Reset + compact output console, no tabs/instructions/feedback panes, save/submit stripped (A7). All state is per-instance React state (a reading hydrates many at once — the singleton chrome store is not shared); the page engine is shared via a common `RunController`, each run in a detached namespace (§6.4). Reading-id event logging + ephemeral-VFS file staging attach with the M2.3 `reader`.
