...
- [ ] CM6 setup: Python language, lint, autocomplete, merge view for history. _(Deps installed in `@blockpy/editor`; implementation next.)_
...
- [ ] Three view modes with legacy toggle semantics; unparseable-text lockout of Blocks/Split (B1–B3) — `blockable` signal ready in `parseSource`; UI wiring pending.
- [ ] Instructor-configurable toolbox from legacy settings keys (B4, fixture A4) — `getFunctionBlock` ported; toolbox presets pending.
