...
const BINOPS_BLOCKLY_DISPLAY_FULL: [string, string][] = BINOPS.map((binop) => [binop[0], binop[1]]);
const BINOPS_BLOCKLY_DISPLAY = BINOPS_BLOCKLY_DISPLAY_FULL.filter(
  (binop) => BINOPS_SIMPLE.indexOf(binop[1]) >= 0,
);
export const BINOPS_AUGASSIGN_DISPLAY_FULL: [string, string][] = BINOPS.map((binop) => [
  binop[4],
  binop[1],
]);
...
const astBinOpConverter = function (this: TextToBlocksConverter, node: ir.BinOp, _parent: unknown) {
